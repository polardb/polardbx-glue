...
public class CharsetMapping {
    ...
    static {
        ...
        MysqlCharset[] charset = new MysqlCharset[] {
            ...
            new MysqlCharset(MYSQL_CHARSET_NAME_eucjpms, 3, 0, new String[] {"EUC_JP_Solaris"},
                new ServerVersion(5, 0, 3)),    // "EUC_JP_Solaris = 	>5.0.3 eucjpms,"
            ...
            // new, Kamenicky encoding usually known as Cp895 but there is no official cp895 specification; close to Cp852, see http://ftp.muni.cz/pub/localization/charsets/cs-encodings-faq
    ...
    public final static String getMysqlCharsetForJavaEncoding(String javaEncoding, ServerVersion version) {
        ...
        List<MysqlCharset> mysqlCharsets =
            CharsetMapping.JAVA_ENCODING_UC_TO_MYSQL_CHARSET.get(javaEncoding.toUpperCase(Locale.ENGLISH));
    ...
    public static boolean isUtf8(int collation) {
        if (collation > 0 && collation < MAP_SIZE) {
            MysqlCharset cs = COLLATION_INDEX_TO_CHARSET[collation];
            // Same object and equals just compare reference.
            return cs.charsetName.equals(MYSQL_CHARSET_NAME_utf8) || cs.charsetName.equals(MYSQL_CHARSET_NAME_utf8mb4);
        }
        return false;
    }

    public static boolean isLatin1(int collation) {
        if (collation > 0 && collation < MAP_SIZE) {
            MysqlCharset cs = COLLATION_INDEX_TO_CHARSET[collation];
            // Same object and equals just compare reference.
            return cs.charsetName.equals(MYSQL_CHARSET_NAME_latin1);
        }
        return false;
    }

    public static String getCollationForCollationIndex(Integer collationIndex) {
        if (collationIndex != null && collationIndex > 0 && collationIndex < MAP_SIZE) {
